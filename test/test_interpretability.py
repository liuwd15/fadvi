...
def test_interpretability_analysis():
    ...
    try:
        ...
        for i, (idx, row) in enumerate(sample_attr.head().iterrows()):
            print(f"  {i+1}. {row['feature']} (mean attribution: {row['attribution_mean']:.6f})")
        
        # Verify the structure of ranked features DataFrame
        expected_columns = {'feature', 'feature_idx', 'attribution_mean', 'attribution_std', 'attribution_abs_mean', 'n_cells'}
        assert expected_columns.issubset(set(sample_attr.columns)), f"Missing expected columns in ranked features"
        assert sample_attr['n_cells'].iloc[0] == len(sample_indices), f"n_cells should match sample size"
    ...
    try:
        ...
        assert expected_columns.issubset(set(sample_attr_gs.columns)), f"Missing expected columns in GradientShap ranked features"
        assert sample_attr_gs['n_cells'].iloc[0] == len(sample_indices), f"n_cells should match sample size"
...
def test_gs_interpretability():
    ...
    try:
        ...
        # Verify structure
        assert isinstance(batch_attr_gs, pd.DataFrame), f"Attributions should be DataFrame with automatic ranking"
        assert batch_attr_gs.shape[0] == adata.n_vars, f"Attribution rows should match genes"
        
        expected_columns = {'feature', 'feature_idx', 'attribution_mean', 'attribution_std', 'attribution_abs_mean', 'n_cells'}
        assert expected_columns.issubset(set(batch_attr_gs.columns)), f"Missing expected columns"
        assert batch_attr_gs['n_cells'].iloc[0] == len(test_indices), f"n_cells should match test indices"
    ...
    try:
        ...
        # Verify structure
        assert isinstance(label_attr_gs, pd.DataFrame), f"Attributions should be DataFrame"
        assert label_attr_gs.shape[0] == adata.n_vars, f"Attribution rows should match genes"
        assert label_attr_gs['n_cells'].iloc[0] == len(test_indices), f"n_cells should match test indices"
    ...
    try:
        ...
        assert isinstance(soft_attr_gs, pd.DataFrame), "Attributions should be DataFrame"
        assert soft_attr_gs['n_cells'].iloc[0] == 10, "n_cells should match subset size"
        
        print(f"✅ Soft prediction GS ranked features DataFrame with {len(soft_attr_gs)} genes")
    ...
    try:
        custom_pred_gs, custom_attr_gs = model.predict(
            ...
            prediction_mode="label", 
            interpretability="gs",
            interpretability_args={"n_samples": 20, "stdevs": 0.1},  # Custom GS parameters
            return_dict=False
        )
        
        print(f"✅ Custom GS args test passed: {custom_attr_gs.shape}")
        assert isinstance(custom_attr_gs, pd.DataFrame), "Custom GS attributions should be DataFrame"
...
def test_interpretability_functionality():
    ...
    try:
        ...
        assert isinstance(batch_attr, pd.DataFrame), f"Attributions should be DataFrame with automatic ranking"
        assert batch_attr.shape[0] == adata.n_vars, f"Attribution rows should match genes (ranked features)"
        expected_columns = {'feature', 'feature_idx', 'attribution_mean', 'attribution_std', 'attribution_abs_mean', 'n_cells'}
        assert expected_columns.issubset(set(batch_attr.columns)), f"Missing expected columns in ranked features"
        
        # Check that we have meaningful attribution data
        assert batch_attr['n_cells'].iloc[0] == len(test_indices), f"n_cells should match test indices"
        print(f"✅ Ranked features DataFrame with {len(batch_attr)} genes and {len(test_indices)} cells")
    ...
    try:
        ...
        # Verify attributions are now a ranked features DataFrame (new behavior)
        assert isinstance(label_attr, pd.DataFrame), f"Attributions should be DataFrame with automatic ranking"
        assert label_attr.shape[0] == adata.n_vars, f"Attribution rows should match genes (ranked features)"
        assert label_attr['n_cells'].iloc[0] == len(test_indices), f"n_cells should match test indices"
    ...
    try:
        ...
        assert isinstance(batch_attr_soft, pd.DataFrame), "Attributions should be DataFrame with automatic ranking"
        assert batch_attr_soft['n_cells'].iloc[0] == 20, "n_cells should match test subset size"
        print(f"✅ Soft prediction ranked features DataFrame with {len(batch_attr_soft)} genes")
    ...
    try:
        ...
        if not predictions_match:
            ...
            if match_ratio >= 0.8:
                ...
                different_indices = np.where(batch_pred_regular != batch_pred_ig_only)[0]
                for i in different_indices[:5]:  # Show first 5 differences
                    print(f"   Cell {i}: Regular={batch_pred_regular[i]}, IG={batch_pred_ig_only[i]}")
                assert match_ratio >= 0.8, f"Prediction match ratio ({match_ratio:.2f}) is too low"
...
def test_interpretability_error_handling():
    ...
    try:
        ...
        try:
            ...
            print("❌ Error handling test failed: Expected ModuleNotFoundError was not raised")
        except ModuleNotFoundError as e:
            # This is what we expect
            print("✅ Error handling test passed - ModuleNotFoundError raised as expected")
...
def test_interpretability_with_different_parameters():
    ...
    assert isinstance(attr1, pd.DataFrame), "Attributions should be DataFrame (ranked features)"
    assert isinstance(attr2, pd.DataFrame), "Attributions should be DataFrame (ranked features)"
    ...
    if not (predictions_match and attributions_close):
        print("Note: Small differences in batch processing are expected for stochastic models")
...
def test_interpretability_quick():
    ...
    print(f"   Batch predictions: {batch_pred.shape}, ranked features: {batch_attr.shape}")
    print(f"   Label predictions: {label_pred.shape}, ranked features: {label_attr.shape}")
    
    # Verify that attributions are now properly ranked DataFrames
    assert isinstance(batch_attr, pd.DataFrame), "Batch attributions should be DataFrame"
    assert isinstance(label_attr, pd.DataFrame), "Label attributions should be DataFrame"
