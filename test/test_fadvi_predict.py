...
def load_data():
    ...
    max_count = np.percentile(adata.X.data if hasattr(adata.X, 'data') else adata.X, 99.9)
    ...
    try:
        ...
        if n_hvg < 100:  # If too few HVGs, be more lenient
            ...
            sc.pp.highly_variable_genes(adata, min_mean=0.001, max_mean=10, min_disp=0.05)
            n_hvg = sum(adata.var['highly_variable'])
        if n_hvg < 50:  # If still too few, use top variable genes with cell_ranger flavor
            print(f"Still only {n_hvg} HVGs found, selecting top 500 most variable genes")
            sc.pp.highly_variable_genes(adata, n_top_genes=500, flavor='cell_ranger')
    except (ValueError, ImportError) as e:
        print(f"Warning: HVG selection failed ({e}), selecting top 500 most variable genes")
        try:
            sc.pp.highly_variable_genes(adata, n_top_genes=500, flavor='cell_ranger')
        except:
    ...
    adata.raw = adata
    adata = adata[:, adata.var.highly_variable].copy()  # Important: copy to avoid view issues
...
def test_predict_method():
    ...
    subset_size = min(500, adata.n_obs // 2)  # Use smaller subset appropriate for synthetic data
    ...
    print(f"Batch probability range: [{pred_batch_soft.values.min():.3f}, {pred_batch_soft.values.max():.3f}]")
    print(f"Batch probability sum check (should be ~1): {pred_batch_soft.sum(axis=1).mean():.3f}")
    
    # Label probabilities
    print(f"Label probability shape: {pred_label_soft.shape}")
    print(f"Label probability range: [{pred_label_soft.values.min():.3f}, {pred_label_soft.values.max():.3f}]")
    print(f"Label probability sum check (should be ~1): {pred_label_soft.sum(axis=1).mean():.3f}")
    ...
    print(f"✅ Soft predictions sum to 1: Batch={pred_batch_soft.sum(axis=1).mean():.3f}, Label={pred_label_soft.sum(axis=1).mean():.3f}")
    ...
    assert pred_batch_hard.shape[0] == adata.n_obs, "Batch predictions should match number of cells"
    assert pred_label_hard.shape[0] == adata.n_obs, "Label predictions should match number of cells"
